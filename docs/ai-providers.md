...
This system uses Google Gemini as the AI provider for content generation. A single `GEMINI_API_KEY` is required. The model is configurable via `plan/config.json` (`gemini_model` field) and defaults to `gemini-2.5-flash-lite`.

本系统使用 Google Gemini 作为内容生成 AI 提供商。仅需一个 `GEMINI_API_KEY`。模型可通过 `plan/config.json` 的 `gemini_model` 字段配置，默认为 `gemini-2.5-flash-lite`。
...
**Cost note:** `gemini-2.5-flash-lite` (~1,400 tokens/day) costs under $0.01/day at current pricing. Google AI Studio provides a free tier.

**费用说明：** `gemini-2.5-flash-lite`（每天约 1,400 tokens）按当前定价低于 $0.01/天。Google AI Studio 提供免费配额。
